...
export default function SingleProduct({ product }: ProductProps) {
  ...
  const [selectedColor, setSelectedColor] = useState(colors?.values[0]);
  const [selectedSize, setSelectedSize] = useState(sizes?.values[0]);
        ...
        <div className="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
          <div className="aspect-w-3 aspect-h-4 hidden overflow-hidden rounded-lg lg:block">
            {product?.media?.nodes?.map((media) => {
              const content = media as unknown as MediaImage;
              return (
                <div
                  key={media.id}
